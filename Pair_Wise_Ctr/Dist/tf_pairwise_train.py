...
def _add_variables_summaries(learning_rate):
  ...
  summaries.append(tf.summary.scalar('training/learning_rate', learning_rate))
  return summaries
...
def triplet_cosine_loss(current, positive, negative, margin, extra=False, scope="triplet_cosine_loss"):
    ...
    def cosine_distance(curr, pair):
      cosine = tf.div(tf.reduce_sum(math_ops.multiply(curr, pair), 1, keep_dims=True),\
    ...
    with tf.name_scope(scope):
      ...
      pair_losses = tf.reduce_mean(math_ops.maximum(0., 2. - (pos_cosine - neg_cosine))) 
      if extra:
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      image_cur = image_preprocessing_fn(image_cur, FLAGS.train_image_height, FLAGS.train_image_width)
      image_pos = image_preprocessing_fn(image_pos, FLAGS.train_image_height, FLAGS.train_image_width)
      image_neg = image_preprocessing_fn(image_neg, FLAGS.train_image_height, FLAGS.train_image_width)

      images_cur, images_pos, images_neg = tf.train.batch(
          [image_cur, image_pos, image_neg],
    ...
    def clone_fn(batch_queue):
      ...
      pair_losses = triplet_loss(cur_logits, pos_logits, neg_logits, 5) 
