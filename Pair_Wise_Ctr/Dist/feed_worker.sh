...
if [ $host_id -lt $ps_num -a $LOCAL_RANK_ID -eq 0 ];then
    ...
    sleep 10
    nohup sh run_multi_train.sh $hosts ps $host_id $ps_num $host_workers 2>&1 > ps_${host_id}.out &
fi

echo "start worker: $local_ip, rank id: $RANK_ID, local rank id: $LOCAL_RANK_ID" 
sleep 10
nohup sh run_multi_train.sh $hosts worker $task_id $ps_num $host_workers 2>&1 > worker_${task_id}.out
