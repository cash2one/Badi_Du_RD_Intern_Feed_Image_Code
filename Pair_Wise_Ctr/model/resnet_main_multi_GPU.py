        ...
        with arg_scope(
            ...
            if include_root_block:
                ...
                with arg_scope(
                    ...
                    net = layers_lib.conv2d(net, 16, 7, stride=2, rate=1, padding='VALID', scope='conv1')
...
def my_resnet(inputs,
    ...
    blocks = [
         ...
         resnet_utils.Block('block3', bottleneck,
                 [(256, 64, 1)] * 22 + [(256, 64, 2)]),
         resnet_utils.Block('block4', bottleneck, [(512, 128, 1)] * 3)
    ]
    return my_resnet_v2(
            inputs,
            blocks,
...
def inference(is_training=True, reuse_variables=None):
    ...
    with arg_scope(resnet_utils.resnet_arg_scope(is_training=is_training)):
        ...
        curr_positive_negative_logits, end_points = my_resnet(curr_positive_negative_images, num_classes=FLAGS.logits, 
            ...
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
                    with arg_scope([variables.variable], device='/cpu:0'):
                        loss = inference(is_training=True,reuse_variables=reuse_variables)
