...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    # Select the dataset #
    ######################
    data_module = dataset_factory.get_module(FLAGS.dataset_name)

    with tf.device(deploy_config.inputs_device()):
      #dataset = dataset_factory.get_dataset(
      #  FLAGS.dataset_name, FLAGS.dataset_split_name, FLAGS.dataset_dir, FLAGS.file_info)
      batch_examples, batch_labels = data_module.get_input(FLAGS.train_data_path, FLAGS.batch_size)

      batch_queue = slim.prefetch_queue.prefetch_queue(
                        [batch_examples, batch_labels], capacity=2*deploy_config.num_clones)
            ...
            with tf.name_scope(deploy_config.clone_scope(i)) as clone_scope:
                with tf.device(clone_device):
                    with tf.variable_scope(tf.get_variable_scope(),
                        ...
                        batch_examples, batch_labels = batch_queue.dequeue()
                        with slim.arg_scope([slim.model_variable, slim.variable],
                                            device=deploy_config.variables_device()):
                            outputs = resnet_vision.inference(batch_examples, batch_labels,  
    ...
    clones = model_deploy.create_clones(deploy_config, data_module.clone_fn,
                                       [batch_queue, network_fn, deploy_config])
