...
def cosine_distance(curr , po_ne):
    ...
    return cosine
'''
def inference(samples, labels, weight_decay, feature_num=None, is_training=True, reuse_variables=None):
    ...
    with arg_scope(resnet_utils.resnet_arg_scope(is_training=is_training, weight_decay=weight_decay)):
        logits = my_resnet(curr_sim_images, weight_decay, num_classes=feature_num, reuse=reuse_variables, scope='Vision')
        ...
        #total_loss = vision_loss + regular_loss
...
def main(_):
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        batch_queue = slim.prefetch_queue.prefetch_queue(
                                [batch_examples, batch_labels], capacity=2 * FLAGS.num_gpus)
            ...
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
                    ...
                    with arg_scope([variables.variable], device='/cpu:0'):
                        loss = inference(batch_examples, batch_labels, is_training=True, reuse_variables=reuse_variables)
