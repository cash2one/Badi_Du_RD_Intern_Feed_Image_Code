...
from datasets import feeding_functions
...
def generator_input_fn(x,
  ...
  meta = next(x(True)) #first get meta, include types and shapes for each colum
  input_keys = meta['keys']
  ...
  def _generator_input_fn():
    ...
    queue = feeding_functions._enqueue_data(
        ...
        num_epochs=num_epochs, meta=meta)
