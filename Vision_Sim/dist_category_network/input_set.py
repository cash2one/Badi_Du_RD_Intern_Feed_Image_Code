...
def get_input(data_path, batch_size):
    ...
    sample_str = tf.decode_base64(sample_b64)
    #sample2_str = tf.decode_base64(sample2_b64)

    sample_tensor = tf.image.resize_image_with_crop_or_pad(tf.image.decode_image(sample_str, channels=c), h, w)
    sample_tensor = tf.reshape(sample_tensor, [h, w, c])
    ...
    sample_tensor = tf.image.per_image_standardization(sample_tensor)
    ...
    '''
    weight = tf.string_to_number(weight, tf.float32)
    ...
    img_batch, label_batch, weight_batch = tf.train.shuffle_batch([sample_tensor, label, weight],\
                              batch_size=batch_size, capacity=batch_size*20,\
                              num_threads=16,\
    ...
    return img_batch, label_batch, weight_batch
