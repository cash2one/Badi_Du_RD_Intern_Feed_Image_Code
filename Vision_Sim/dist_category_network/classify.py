...
tf.app.flags.DEFINE_float('lrn_rate', 0.01, 'Initial learning rate.')
...
tf.app.flags.DEFINE_string(
    'train_data_path', None, 'The directory where the dataset files are stored.')

tf.app.flags.DEFINE_string(
    'file_info', None, 'The file pattern or kv path.')
...
tf.app.flags.DEFINE_integer(
    'train_image_height', None, 'Train image size')
tf.app.flags.DEFINE_integer(
    'train_image_width', None, 'Train image size')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      #  FLAGS.dataset_name, FLAGS.dataset_split_name, FLAGS.dataset_dir, FLAGS.file_info)
      batch_examples, batch_labels, batch_weights = input_set.get_input(FLAGS.train_data_path, FLAGS.batch_size)

      batch_queue = slim.prefetch_queue.prefetch_queue(
                        [batch_examples, batch_labels, batch_weights], capacity=2*deploy_config.num_clones)
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      def proc():
        ...
        img_cur = image_preprocessing_fn(img_cur, FLAGS.train_image_height, FLAGS.train_image_width)
        img_pos = image_preprocessing_fn(img_pos, FLAGS.train_image_height, FLAGS.train_image_width)
        img_neg = image_preprocessing_fn(img_neg, FLAGS.train_image_height, FLAGS.train_image_width)
      ...
      batch_queue = slim.prefetch_queue.prefetch_queue(
          [images_cur, images_pos, images_neg], capacity=2 * deploy_config.num_clones)
    ...
    def clone_fn(batch_queue):
      ...
      tf.losses.compute_weighted_loss(pair_losses)
    ...
    def create_clones(deploy_config, batch_queue):
        clones = []
            ...
            with tf.name_scope(deploy_config.clone_scope(i)) as clone_scope:
                with tf.device(clone_device):
                    with tf.variable_scope(tf.get_variable_scope(),
                        ...
                        batch_examples, batch_labels, batch_weights = batch_queue.dequeue()
                        with slim.arg_scope([slim.model_variable, slim.variable],
                                            device=deploy_config.variables_device()):
                            outputs = resnet_vision.inference(batch_examples, batch_labels, batch_weights,  
                                                           FLAGS.weight_decay, feature_num=FLAGS.logits, 
                                                           is_training=True, reuse_variables=reuse_variables)
    ...
    clones = create_clones(deploy_config, batch_queue)
